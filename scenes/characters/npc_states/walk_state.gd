...
func _ready() -> void:
	...
	call_deferred("character_setup") # To allow calling the function after the current frame is processed
...
func _on_physics_process(_delta : float) -> void:
	...
	# The if else above substitute the following lines
	#character.velocity = target_direction * speed # we are not using decter because character will use it internaly in the move and slide
	#character.move_and_slide()
